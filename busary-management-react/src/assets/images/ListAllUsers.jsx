...
const http = new HttpFunction();
 ...
 const fetchData = async (currentPage_, searchText_) => {
  setLoading(true);
  var url = `/api/v2/user?page=${currentPage_ - 1}&size=10&sort=firstName&searchText=${searchText_}`;
...
const fetchHistoryData = async () => {
  var url = `/api/v2/history?page=0&size=10&sort=historyId,DESC&searchText=`;
...
const fetchRegionData = async (_provinceId) => {
  var url = `/api/v2/region/find-by-provinceId?provinceId=${_provinceId}`;
  const apiCall = await http.get(url);
  if(apiCall.code === 200) {
   
    setRegionList(apiCall.data);
...
// Fetch province data from server
const fetchProvinceData = async () => {
  const apiCall = await http.get(`/api/v2/province`);
  if(apiCall.code === 200) {
    setProvinceList(apiCall.data.results);
  } else {
